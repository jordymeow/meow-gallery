...
import { useNekoColors, NekoPaging, NekoIcon, NekoButton, NekoTypo, NekoInput, NekoTable, NekoModal, nekoFetch, NekoSelect, NekoOption, NekoSpacer, NekoSwitch } from '@neko-ui';
...
const ShortcodeMaker = ({
    ...
    const cleanCancel = () => { setModals({ ...modals, createShortcode: false }); setselectedMedias({ thumbnail_ids: [], thumbnail_urls: [], thumbnails: [] }); setGalleryName(''); setGalleryLayout(''); setGalleryId(''); setGalleryDescription(''); setIsPostMode(false); setIsLatestPostsMode(true); setPostIds([]); setLatestPostsNumber(5);};
    ...
    const onEditShortcode = (id) => {
        ...
        setIsLatestPostsMode(!gallery.latest_posts);
        if (gallery.posts) setPostIds(gallery.posts);
        if (gallery.latest_posts !== undefined) setLatestPostsNumber(gallery.latest_posts);
    ...
    const fetchSavedGalleries = async () => {
        setBusyAction(true);
        const params = {...shortcodesQueryParams};
        params.offset = (shortcodesQueryParams.page - 1) * shortcodesQueryParams.limit;
    ...
    const rows = useMemo(() => {
        return Object.entries(savedGalleries)?.map(([id, gallery]) => {
            ...
            if (gallery && gallery.is_post_mode) {
                if (gallery.posts) {
                ...
                if (gallery.medias && gallery.medias.thumbnail_ids)
            ...
            const thumbnail = <>
                <div style={{ width: 100, display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 3 }}>
                    {gallery.medias && gallery.medias.thumbnails && gallery.medias.thumbnails.slice(0, 4).map((thumbnail, index) => {
                        const el = thumbnail.mime && thumbnail.mime.includes('video') ?
                        <video autoPlay muted loop playsInline key={index} src={thumbnail.url} style={{ width: '100%', height: '100%', display: 'block', borderRadius: 3, objectFit: 'cover' }} /> :
    ...
    const jsxShortcodePaging = useMemo(() => {
        ...
        <div style={{ display: 'flex', flexDirection: 'row' }}>
            ...
            total={shortcodesTotal} onClick={page => { 
                setShortcodesQueryParams(x => ({ ...x, page }));
            }}
            />
        </div>
        </div>);
    ...
    const jsxShortcodeMakerTable = <>
        ...
        <NekoTable
            ...
            onSortChange={(accessor, by) => {
              setShortcodesQueryParams(x => ({ ...x, sort: { accessor, by } }));
            }}
            filters={filters}
            onFilterChange={(accessor, value) => {
              ...
              setShortcodesQueryParams(x => ({ ...x, page: 1, filters: freshFilters }));
            }}
    ...
    const jsxSelectImagesModal = <MediaSelector
        ...
        onSave={(medias) => { setselectedMedias(medias); }}
    />;
        ...
        <NekoModal
            ...
            content={<>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <NekoInput name="gallery_name" type="text" value={galleryName} placeholder="Gallery Name..." onChange={(e) => setGalleryName(typeof e === 'string' ? e : '')} style={{ flex: .9 }} />
                ...
                { isPostMode && <>
                    <div style={{ display: 'flex', alignItems: 'center' }}>
                        ...
                        { !isLatestPostsMode && 
                            ...
                            placeholder="Post IDs... (separated by commas)" onChange={(e) => setPostIdsStringToArray(typeof e === "string" ? e : '')} />
                        }
                ...
                <NekoInput name="gallery_description" type="text" value={galleryDescription} placeholder="Gallery Description..." onChange={(e) => setGalleryDescription(typeof e === "string" ? e : "")} />
