...
const { BlockControls, MediaPlaceholder, InspectorControls, useBlockProps } = wp.blockEditor;
...
class GalleryEdit extends Component {
	...
	render() {
		...
		if ( window.mgl_meow_gallery && window.mgl_meow_gallery.galleries ) {
			const data = mgl_meow_gallery.galleries['galleries'];
			let galleries = ( data ? Object.keys( data ) : [] ).map(
				x => {
					return {
						label: data[x].name,
						value: x,
		...
		if ( window.mgl_meow_gallery && window.mgl_meow_gallery.collections ) {
			const data = mgl_meow_gallery.collections['collections'];
			let collections = ( data ? Object.keys( data ) : [] ).map(
				x => {
					return {
						label: data[x].name,
						value: x,
				...
				{!error && hasImagesToShow && !htmlPreview && (
					<div style={{
						...
						<p style={{
							...
							Loading Gallery... (<a style={{cursor: 'pointer', color: '#1e90ff'}}
								onClick={() => this.onRefresh()}>click here</a> to force refresh)
						</p>
						<style>{`
							@keyframes pulse {
								0%, 100% { opacity: 0.6; }
...
const GalleryEditWithNotices = withNotices( GalleryEdit );
