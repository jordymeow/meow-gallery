...
class Meow_MGL_Builders_Horizontal extends Meow_MGL_Builders_Core {
	...
	function inline_css() {
		...
		$gutter = isset( $this->atts['gutter'] ) ? $this->atts['gutter'] : get_option( 'mgl_horizontal_gutter', 10 );
		$image_height = isset( $this->atts['image_height'] ) ? $this->atts['image_height'] : get_option( 'mgl_horizontal_image_height', 500 );
		$hide_scrollbar = isset( $this->atts['hide_scrollbar'] ) ? $this->atts['hide_scrollbar'] : get_option( 'mgl_horizontal_hide_scrollbar', false );
	...
	function build( $idsStr ) {
		...
		$gutter = isset( $this->atts['mgl_horizontal_gutter'] ) ?
			$this->atts['mgl_horizontal_gutter'] : get_option( 'mgl_horizontal_gutter', 5 );
		$hide_scrollbar = isset( $this->atts['hide_scrollbar'] ) ? $this->atts['hide_scrollbar'] :
			get_option( 'mgl_horizontal_hide_scrollbar', false );
