...
const Dashboard = () => {
  ...
  const { data: settings, error: swrError, isLoading } = useQuery(['all_settings'], fetchSettings);
  const { mutate: mutateUpdateOption } = useMutation(updateOption, {
    onSuccess: () => {
      queryClient.invalidateQueries(['all_settings']);
    },
  });

  const { mutate: loadErrorLogs, data: phpErrorLogs = [] } = useMutation(fetchErrorLogs, {
  ...
  const jsxHideMeowApps = 
    <NekoSettings title="Main Menu">
      <NekoCheckboxGroup max="1">
        <NekoCheckbox name="meowapps_hide_meowapps" label="Hide (Not Recommended)" description={<NekoTypo p>This will hide the Meow Apps Menu (on the left side) and everything it contains. You can re-enable it through though an option that will be added in Settings &rarr; General.</NekoTypo>} value="1" disabled={busy} checked={hide_meowapps} onChange={handleUpdateOption} />
      </NekoCheckboxGroup>
    </NekoSettings>;
  ...
  const jsxForceSSLVerify = 
    <NekoSettings title="SSL Verify">
      <NekoCheckboxGroup max="1">
        <NekoCheckbox name="force_sslverify" label="Force (Not Recommended)" description={<NekoTypo p>This will enforce the usage of SSL when checking the license or updating the plugin.</NekoTypo>} value="1" disabled={busy} checked={force_sslverify} onChange={handleUpdateOption} />
      </NekoCheckboxGroup>
    </NekoSettings>;
        ...
        <NekoColumn full>
          <NekoTabs keepTabOnReload={true}>
            ...
            <NekoTab title="PHP Error Logs">
              <TabText>
                <NekoButton style={{ marginBottom: 10 }} color={'#ccb027'} onClick={handleLoadErrorLogs} disabled={busy} isBusy={busy === 'isLoadingErrorLogs'}>
                  Load PHP Error Logs
                </NekoButton>
                <StyledPhpErrorLogs>
